...
public class Alipaycontroller {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "doAlipay", produces = "text/html; charset=UTF-8")
    protected String doAlipay(String costid, String money) throws IOException {
        System.out.println("支付接口, 进入接口*******");
        System.out.println("前往支付宝第三方网关进行支付");
        log.info("前往支付宝第三方网关进行支付");
        ...
        //trade_no和out_trade_no二选一
        //交易编号
        String out_trade_no = String.valueOf(Utils.outTradeNo());
        // 交易编号
        String trade_no = costid;
        //订单名称，必填
        String subject = "物业费";
    ...
    public ModelAndView alipayReturnNotice(HttpServletRequest request, HttpServletRequest response) throws Exception {
        ...
        if (signVerified) {
            ...
            ModelAndView mv = new ModelAndView("customer_index");
            // 修改叮当状态，改为 支付成功，已付款; 同时新增支付流水
            int costState = lCostService.updateCostState(Integer.parseInt(out_trade_no));
            int insertpay = laliPayService.insert(Integer.parseInt(out_trade_no));
            ...
            if (costState > 0 && insertpay > 0) {
                log.info("支付成功",costState);
                return  mv;//"redirect:/statics/html/customer_index.html";
            }
		...
		*/
        if (signVerified) {//验证成功
            ...
            if (trade_status.equals("TRADE_FINISHED")) {
                ...
                int costState = lCostService.updateCostState(Integer.parseInt(out_trade_no));
                log.info("缴费状态", costState);
                int insertpay = laliPayService.insert(Integer.parseInt(out_trade_no));
                log.info("流水状态", insertpay);
